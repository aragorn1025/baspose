...
import numpy as np
def row_count():
   connection = pymysql.connect(host='140.115.51.184',
                                port=40136,
                                user = 'cilab',
...
def layup(id, step1, traveling, straight):
    ...
    try:
        with connection.cursor() as cursor:
            cursor = connection.cursor()
            sql = "INSERT INTO layup(id, step1, traveling, straight) VALUES (%s, %s, %s, %s)"
            cursor.execute(sql, (id, step1, traveling, straight))
            connection.commit()
